...
describe("Uptime", function(){
  it("Should list of valid set of uptime", function(done){

    ubivar.uptime.list(function(err, res){
  ...
  it("Should filter uptime based on timestamp", function(done){
    var begin = "2015-03-30 23:00:00"
      , end   = "2015-03-30 24:00:00"

    ubivar.uptime.list({"timestamp":{ "gte":begin , "lte":end}}, function(err, res){
      ...
      _.each(results, function(result){
        ...
        if((time-begin)<0 || (end-time)<0){
          return done(new Error("Should return uptimes within ["+begin,",",end+"]: ", time))
        }
      })
  ...
  it("Should summary uptimes", function(done){
    ubivar.uptime.summary(function(err, res){
      if(err){
        console.log(err)
        return done(err)
      } else if(res.data.length === 0){
